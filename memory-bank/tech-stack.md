...
| 技术                    | 版本                  | 用途           |
| --------------------- | ------------------- | ------------ |
| WinUI 3               | Windows App SDK 2.x | Windows 原生界面 |
| .NET                  | 9                   | Runtime      |
| C#                    | 13                  | 开发语言         |
| XAML                  | WinUI3              | UI 描述        |
| CommunityToolkit.Mvvm | 8.x                 | MVVM 源代码生成   |

负责：窗口管理、页面布局、文件选择器、拖放、模板管理、进度显示、动画、标题栏、键盘快捷键。
...
│
├── backend/                      # FastAPI 后端
│   ├── server.py                 # 入口 + 路由注册 + CORS
│   ├── routes/                   # API 端点
│   │   ├── profile.py            # 排版配置 CRUD
│   │   ├── files.py              # 文件管理
│   │   └── format_tasks.py       # 排版任务管理
│   ├── services/                 # 业务逻辑
│   ├── engine/                   # 排版引擎（核心）
│   │   ├── formatter.py          # 排版主控
│   │   ├── page.py               # 页面规则
...
| POST   | `/api/files/select`         | 添加文件   |
| POST   | `/api/files/folder`         | 导入文件夹  |
| POST   | `/api/files/remove`         | 移除文件   |
| DELETE | `/api/files/all`            | 清空文件   |
| POST   | `/api/format/start`         | 启动排版   |
| GET    | `/api/format/{id}/progress` | 查询进度   |
| GET    | `/api/format/{id}/result`   | 获取结果   |
